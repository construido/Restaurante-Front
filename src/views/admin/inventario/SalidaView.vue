    ...
    setup(){
        ...
        function sumaTotal(datos){
            for(var i=0; i<datos.length; i++) {
                total.value = total.value + datos[i].Monto_Parcial_Venta
                totalRecords.value = totalRecords.value + datos[i].Cantidad_Venta
            }

            total.value = formatCurrency(total.value) ? formatCurrency(total.value) : 0
        }
        function listar(){
            producto.salida(lazyParams.value, filters.value)
            .then(res => {
                loading.value = true
                setTimeout(() => {
